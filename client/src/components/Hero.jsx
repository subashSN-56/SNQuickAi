...
const Hero = () => {
    ...
    return (
        <div
            className='px-4 sm:px-20 xl:px-32 relative inline-flex flex-col w-full justify-center bg-cover bg-center bg-no-repeat min-h-screen bg-[url(/assets/gradientBackground.png)]'
        ...
        >
            <div className='text-center mb-6'>
                ...
                <p className='mt-4 max-w-xs sm:max-w-lg 2xl:max-w-xl m-auto max-sm:text-xs text-gray-600' >Transform your content creation with our suite of premium AI tools.
                    Write articles , generate images, and create videos effortlessly.
                </p>
            </div>
            <div className='flex flex-wrap justify-center gap-4 text-sm max-sm:text-xs'>
                <button onClick={() => navigate('/ai')} className='bg-primary text-white px-6 py-3 rounded-full hover:bg-primary/90 transition-colors duration-300 cursor-pointer flex items-center gap-2'>
                    State creating now
                    <span className='text-amber-400 font-semibold'>  Get Started</span>
                </button >
                <button className='bg-primary text-white px-6 py-3 rounded-full hover:bg-primary/90 transition-colors duration-300 cursor-pointer flex items-center gap-2'>
                    Watch Now
                    <span className='text-amber-400 font-semibold'>  Click</span>
                </button>
            </div>
            <div className='flex items-center justify-center gap-2 mt-6 text-gray-500 text-xs sm:text-sm'>
                <img src={assets.user_group} alt='' className='h-8' /> Trust by 10 K+ users
            </div>
        </div>
    )
}
